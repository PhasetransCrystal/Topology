package net.ptcrys.topo.api.api.async;
...
public final class TopoAsyncExecutors {

    private static final Logger LOGGER = LoggerFactory.getLogger(TopoAsyncExecutors.class);
    ...
    private TopoAsyncExecutors() {}

    public static void register() {
        synchronized (TopoAsyncExecutors.class) {
            if (registered) {
                return;
            }
            registered = true;
        }
        NeoForge.EVENT_BUS.addListener(TopoAsyncExecutors::onServerStopped);
    }
    ...
    public static synchronized ScheduledExecutorService tickScheduler() {
        if (tickScheduler == null || tickScheduler.isShutdown() || tickScheduler.isTerminated()) {
            ScheduledThreadPoolExecutor executor = new ScheduledThreadPoolExecutor(
                    ...
                    new NamedThreadFactory("topo-tick-async-", Thread.NORM_PRIORITY));
    ...
    private static final class NamedThreadFactory implements ThreadFactory {
        ...
        public Thread newThread(Runnable task) {
            ...
            thread.setUncaughtExceptionHandler((failedThread, error) -> LOGGER.error(
                    "[TopoAsyncExecutors] {} terminated with an uncaught exception",
