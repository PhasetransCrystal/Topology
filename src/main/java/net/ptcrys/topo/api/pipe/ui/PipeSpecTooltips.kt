...
object PipeSpecTooltips {
    ...
    fun createSpecPanel(definition: PipeDefinition): UIElement {
        ...
        lcd.setId("oi_pipe_spec_tooltip")
        lcd.addStaticEntry(
            BuiltinOIPipeLang.TOOLTIP_PIPE_MAX_RATE.getComponent(),
            perTick(definition, definition.maxExtractRate()),
            LcdData.LED_TEXT,
        )
        lcd.addStaticEntry(
            BuiltinOIPipeLang.TOOLTIP_PIPE_NODE_THROUGHPUT.getComponent(),
            perTick(definition, definition.nodeThroughput()),
            LcdData.LED_TEXT,
        )
        lcd.addStaticEntry(
            BuiltinOIPipeLang.TOOLTIP_PIPE_STRATEGIES.getComponent(),
        ...
        lcd.addStaticEntry(
            BuiltinOIPipeLang.TOOLTIP_PIPE_FILTER.getComponent(),
    ...
    private fun perTick(definition: PipeDefinition, amount: Int): Component = BuiltinOIPipeLang.TOOLTIP_PIPE_PER_TICK.getComponent(definition.profile().formatAmount(amount.toLong()))

    /** 策略行右列:可用策略数量(测试 292)。 */
    @JvmStatic
    fun strategyCountValue(count: Int): Component = BuiltinOIPipeLang.TOOLTIP_PIPE_STRATEGY_COUNT.getComponent(count)
    ...
    private fun filterSpec(definition: PipeDefinition): Component {
        ...
        if (!filter.enabled()) {
            return BuiltinOIPipeLang.TOOLTIP_PIPE_FILTER_NONE.getComponent()
        }
        return if (filter.allowTags) {
            BuiltinOIPipeLang.TOOLTIP_PIPE_FILTER_ENTRIES_TAGS.getComponent(filter.entryCapacity())
        } else {
            BuiltinOIPipeLang.TOOLTIP_PIPE_FILTER_ENTRIES.getComponent(filter.entryCapacity())
        }
    }
}
