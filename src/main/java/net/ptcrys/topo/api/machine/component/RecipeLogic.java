...
import net.ptcrys.topo.api.machine.data.DataLong;
...
/** Minimal server-side state machine for one or more Topo recipe types. */
public class RecipeLogic extends MachineTicker
    ...
    private final List<TopoRecipeType<?>> recipeTypes;
    ...
    private final DataLong activeParallelFactor = data().longField("activeParallelFactor", 0L)
            ...
            .syncNone()
            .done();
    private @Nullable RecipeHolder<? extends TopoRecipe> activeRecipe;
    ...
    private @Nullable TopoRecipe activeRecipeView;
    /** Machine-bound direct tick kernel; rebuilt only when the recipe routing table changes. */
    private TopoRecipe.@Nullable BoundTickIoPlan activeTickIoPlan;
    ...
    public static ComponentMount<RecipeLogic> mount(
                                                    ...
                                                    TopoRecipeType<?>... recipeTypes) {
        List<TopoRecipeType<?>> types = List.of(recipeTypes);
    ...
    public void tick(long gameTime, TickHandle handle) {
        TopoRecipe workingRecipe = activeRecipeView;
    ...
    private boolean tryStartRecipe() {
        ...
        while (true) {
            TopoRecipe recipe;
            TopoRecipe.TickIoResult startTickIo;
            ...
            try (MachineComponents.RecipePoolScope poolScope = components.openRecipePool(candidate.poolId())) {
                ...
                if (startTickIo == TopoRecipe.TickIoResult.SUCCESS && !recipe.canEmitOutputs(machine())) {
                    startTickIo = TopoRecipe.TickIoResult.OUTPUT_BLOCKED;
                }
                candidate.requireCurrentRevision();
                if (startTickIo == TopoRecipe.TickIoResult.SUCCESS && recipe.consumeInputs(machine())) {
            ...
            if (startTickIo != TopoRecipe.TickIoResult.SUCCESS) {
    ...
    private void processWorkingTick(TopoRecipe view) {
        TopoRecipe.TickIoResult tickIo = handleActiveTickIo(view);
        if (tickIo != TopoRecipe.TickIoResult.SUCCESS) {
    ...
    private boolean retryTickIo() {
        TopoRecipe view = activeRecipeView;
        if (view == null) {
            resetToIdle();
            return false;
        }
    ...
    private boolean retryProcessTickIo(TopoRecipe view) {
     ...
     */
    private TopoRecipe.TickIoResult handleActiveTickIo(TopoRecipe recipe) {
        if (!recipe.hasTickIo()) {
            return TopoRecipe.TickIoResult.SUCCESS;
        }
        ...
        try {
            ...
            TopoRecipe.BoundTickIoPlan plan = activeTickIoPlan;
            if (plan != null && plan.routingRevision() == routingRevision) {
                TopoRecipe.TickIoResult directResult = plan.tryHandleDirect();
    ...
    private void advanceProgress(TopoRecipe recipe) {
        progress.inc();
    ...
    private boolean waitToStart(
                                TopoRecipe recipe,
                                TopoRecipe.TickIoResult reason,
                                long resourceVersion,
                                long searchRevision,
                                boolean searchCacheable) {
        State nextState = reason == TopoRecipe.TickIoResult.INPUT_BLOCKED ? State.WAITING_TICK_INPUT_TO_START : State.WAITING_TICK_OUTPUT_TO_START;
        progress.set(0);
        maxProgress.set(0);
        clearActiveRecipe();
        setState(nextState);
    ...
    private void waitDuringProcessing(TopoRecipe recipe, TopoRecipe.TickIoResult reason) {
        State nextState = reason == TopoRecipe.TickIoResult.INPUT_BLOCKED ? State.WAITING_TICK_INPUT_TO_PROCESS : State.WAITING_OUTPUT;
        setState(nextState);
    ...
    private TopoRecipe applyRecipeModifiers(TopoRecipe recipe) {
    ...
    private boolean resolveActiveRecipeIfNeeded() {
        ...
        RecipeHolder<? extends TopoRecipe> current = activeRecipe;
        if (current != null) {
        ...
        RecipeHolder<? extends TopoRecipe> resolved = resolveSupportedRecipe(level.getServer(), recipeId);
        if (resolved == null) {
            enterMissingActiveRecipe(currentState);
            return false;
        }
        TopoRecipe recipe = resolved.value();
        ...
        TopoRecipe view = applyRecipeModifiers(recipe);
    ...
    @SuppressWarnings("unchecked")
    private @Nullable RecipeHolder<? extends TopoRecipe> resolveSupportedRecipe(
        ...
        for (TopoRecipeType<?> recipeType : recipeTypes) {
            ...
            if (resolved != null && resolved.value() instanceof TopoRecipe recipe && supportsRecipeType(recipe.recipeType())) {
                return (RecipeHolder<? extends TopoRecipe>) resolved;
            }
        }
        return null;
    }

    private boolean supportsRecipeType(TopoRecipeType<?> recipeType) {
        for (TopoRecipeType<?> supported : recipeTypes) {
    ...
    private void cacheBlockedStartIfStable(
        ...
        // Principle: Topo semantics already stop at the first input-matching recipe; if its tick
    ...
    private void cacheTickIoBlockedIfStable(TopoRecipe recipe) {
    ...
    private void cacheOutputBlockedIfStable(TopoRecipe recipe) {
    ...
    public enum State {
        ...
        public net.minecraft.network.chat.Component displayName() {
            return switch (this) {
                ...
                case WORKING -> TopoApiLang.UI_RECIPE_STATE_WORKING.getComponent();
                case WAITING_OUTPUT -> TopoApiLang.UI_RECIPE_STATE_WAITING_OUTPUT.getComponent();
                case WAITING_TICK_INPUT_TO_PROCESS -> TopoApiLang.UI_RECIPE_STATE_WAITING_TICK_INPUT_TO_PROCESS.getComponent();
                case WAITING_TICK_INPUT_TO_START -> TopoApiLang.UI_RECIPE_STATE_WAITING_TICK_INPUT_TO_START.getComponent();
                case WAITING_TICK_OUTPUT_TO_START -> TopoApiLang.UI_RECIPE_STATE_WAITING_TICK_OUTPUT_TO_START.getComponent();
                case MISSING_ACTIVE_RECIPE -> TopoApiLang.UI_RECIPE_STATE_MISSING_ACTIVE_RECIPE.getComponent();
            };
        }
