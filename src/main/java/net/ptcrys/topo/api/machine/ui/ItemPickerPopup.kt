...
object ItemPickerPopup {
    ...
    fun open(host: UIElement, title: Component, entryFor: Function<ItemStack, String?>, previewIcon: Function<String, IGuiTexture>, onConfirm: Consumer<List<String>>, initialEntries: List<String> = emptyList()) {
        ...
        MachineUiContainerTemplate.openModalPopup(
            ...
            val stagingRow = slotRow("topo_item_picker_staging_row")
            ...
            val invGrid = MachineUiLayout.column(gap = 0f, id = "topo_item_picker_inventory") {
                for (rowIndex in 0 until 3) {
                    val row = slotRow("topo_item_picker_inv_row_$rowIndex")
                ...
                val hotbar = slotRow("topo_item_picker_hotbar").apply {
            ...
            val actionRow = MachineUiLayout.row(
                ...
                id = "topo_item_picker_action_row",
            ) {
                add(
                    actionButton(
                        "topo_item_picker_cancel",
    ...
    private fun inventoryCell(index: Int, stack: ItemStack, onPick: (ItemStack) -> Unit): UIElement {
        val cell = slotCell("topo_item_picker_inv_$index", MachineUiComponentStyle.realSlotTexture())
        if (!stack.isEmpty) {
            cell.addChild(iconElement("topo_item_picker_inv_icon_$index", ItemStackTexture(stack.copy()), ICON))
    ...
    private fun divider(): UIElement = MachineUiLayout.horizontalDivider(height = 1f, id = "topo_item_picker_divider")
    ...
    private class StagingCell(index: Int, private val previewIcon: Function<String, IGuiTexture>, onClear: () -> Unit) {
        private val icon = UIElement().apply {
            setId("topo_item_picker_staging_icon_$index")
            isAllowHitTest = false
            layout {
                it.width(ICON)
                it.height(ICON)
        ...
        private val plus = UIElement().apply {
            setId("topo_item_picker_staging_plus_$index")
            isAllowHitTest = false
            layout {
                it.width(PLUS_ICON)
                it.height(PLUS_ICON)
        ...
        val root: UIElement = UIElement().apply {
            setId("topo_item_picker_staging_$index")
            layout {
                it.width(SLOT)
                it.height(SLOT)
                it.flexShrink(0f)
