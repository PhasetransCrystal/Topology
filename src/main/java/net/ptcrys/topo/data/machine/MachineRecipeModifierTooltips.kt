...
object MachineRecipeModifierTooltips {
    ...
    fun createPanel(definition: MachineDefinition): UIElement {
        ...
        lcd.setId("topo_machine_recipe_modifier_tooltip")
    ...
    fun panelRows(displays: List<RecipeModifierDisplay>): List<Row> {
        ...
        val rows = mutableListOf(
            Row(
                BuiltinTopoMachineUiLang.TOOLTIP_MACHINE_RECIPE_MODIFIERS.getComponent(),
                BuiltinTopoMachineUiLang.TOOLTIP_MACHINE_MODIFIER_COUNT.getComponent(displays.size),
                false,
            ),
        )
