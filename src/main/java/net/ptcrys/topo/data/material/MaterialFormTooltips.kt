 ...
 *
 * 面板纯注册期事实,逐物品恒定 —— 用 [net.ptcrys.topo.api.machine.ui.tooltip.TopoTooltipUiProvider]
 * 默认缓存键(item 实例)。
 */
object MaterialFormTooltips {
    ...
    fun createPanel(material: Material, form: MaterialForm): UIElement {
        ...
        lcd.setId("topo_material_form_tooltip")
        val massPerUnit = material.strategy()
            .data(BuiltinTopoMaterialDataTypes.MASS)
            .map { it.value() }
            .orElse(null)
        val amountUnits = form.strategy()
            .data(BuiltinTopoFormDataTypes.AMOUNT)
            .orElse(null)
    ...
    fun panelRows(materialName: Component, formName: Component, amountUnits: Int?, massPerUnit: Int?): List<Pair<Component, Component>> {
        val rows: MutableList<Pair<Component, Component>> = mutableListOf(
            BuiltinTopoMaterialFormLang.TOOLTIP_MATERIAL_MATERIAL.getComponent() to materialName,
            BuiltinTopoMaterialFormLang.TOOLTIP_MATERIAL_FORM.getComponent() to formName,
        )
        if (amountUnits != null) {
            rows += BuiltinTopoMaterialFormLang.TOOLTIP_MATERIAL_AMOUNT.getComponent() to amountText(amountUnits)
        }
        if (amountUnits != null && massPerUnit != null) {
            rows += BuiltinTopoMaterialFormLang.TOOLTIP_MATERIAL_MASS.getComponent() to
    ...
    private fun amountText(units: Int): Component = when {
        units % FormAmountDataType.UNITS_PER_INGOT != 0 ->
            BuiltinTopoMaterialFormLang.TOOLTIP_MATERIAL_AMOUNT_UNITS.getComponent(units)

        units == FormAmountDataType.UNITS_PER_INGOT ->
            BuiltinTopoMaterialFormLang.TOOLTIP_MATERIAL_AMOUNT_INGOT.getComponent(1)

        else ->
            BuiltinTopoMaterialFormLang.TOOLTIP_MATERIAL_AMOUNT_INGOTS.getComponent(
                units / FormAmountDataType.UNITS_PER_INGOT,
            )
    }
}
