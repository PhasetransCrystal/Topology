...
final class IndexedOIRecipe<R extends OIRecipe> {
    ...
    static <R extends OIRecipe> IndexedOIRecipe<R> of(
                                                      int ordinal,
                                                      RecipeHolder<R> holder,
        ...
        // Optimization: index only the start-input phase used by OIRecipeType#findRecipe.
        ...
        for (var entry : reservations.int2LongEntrySet()) {
            amounts.mergeLong(entry.getIntKey(), entry.getLongValue(), IndexedOIRecipe::saturatedAdd);
        }
        ...
        return new IndexedOIRecipe<>(ordinal, holder, keys, keyAmounts, keys.length == 0);
    }

    @SuppressWarnings({ "rawtypes", "unchecked" })
    private static void addEntries(
                                   OIRecipe.InputEntry<?>[] entries,
                                   RecipeSearchKeyRegistry registry,
            ...
            for (Object content : entry.contents()) {
                ...
                for (Object key : keys) {
                    ...
                    if (amountMode == RecipeCapability.InputIndexAmountMode.MAX_RESERVATION) {
                        ...
                        additiveAmounts.mergeLong(keyId, amount, IndexedOIRecipe::saturatedAdd);
                    }
                }
            }
        }
