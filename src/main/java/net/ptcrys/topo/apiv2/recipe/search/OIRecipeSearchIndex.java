...
/**
 * Capability-aware search index for one OI recipe type.
 *
 * <p>
 ...
 */
public final class OIRecipeSearchIndex<R extends OIRecipe> {
    ...
    private final List<IndexedOIRecipe<R>> recipesByOrdinal;
    ...
    private OIRecipeSearchIndex(
                                RecipeSearchKeyRegistry keyRegistry,
                                List<IndexedOIRecipe<R>> recipesByOrdinal,
    ...
    public static <R extends OIRecipe> OIRecipeSearchIndex<R> build(Collection<RecipeHolder<R>> holders) {
        return build(holders, 0);
    }

    public static <R extends OIRecipe> OIRecipeSearchIndex<R> build(
        ...
        List<IndexedOIRecipe<R>> recipes = new ArrayList<>(holders.size());
        ...
        for (RecipeHolder<R> holder : holders) {
            IndexedOIRecipe<R> recipe = IndexedOIRecipe.of(ordinal++, holder, registry, extractableCapabilities);
            recipes.add(recipe);
        ...
        int indexedCount = 0;
        for (IndexedOIRecipe<R> recipe : recipes) {
        ...
        return new OIRecipeSearchIndex<>(
                registry,
                recipes,
    ...
    private static <R extends OIRecipe> RecipeHolder<R>[] holdersByOrdinal(List<IndexedOIRecipe<R>> recipes) {
    ...
    private @Nullable RecipeHolder<R> findFirstPreciseCandidate(MachineBlockEntity machine) {
        for (int ordinal : preciseCandidateOrdinals) {
            IndexedOIRecipe<R> recipe = recipesByOrdinal.get(ordinal);
    ...
    private @Nullable RecipeHolder<R> findFirstStreamingCandidate(MachineBlockEntity machine, SearchScratch scratch) {
        ...
        while (true) {
            ...
            if (preciseOrdinal < indexedOrdinal) {
                IndexedOIRecipe<R> recipe = recipesByOrdinal.get(preciseOrdinal);
                preciseIndex++;
                if (matchesStartInputs(recipe.holder.value(), machine)) {
                    return recipe.holder;
                }
            ...
            IndexedOIRecipe<R> recipe = recipesByOrdinal.get(indexedOrdinal);
    ...
    private static boolean matchesStartInputs(OIRecipe recipe, MachineBlockEntity machine) {
        // Optimization: call OIRecipe's start-input matcher directly instead of allocating the
        // vanilla RecipeInput adapter. Principle: this preserves the old OIRecipe#matches
