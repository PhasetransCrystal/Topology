...
/**
 * Path helpers for vanilla-facing OI types: OI recipe names must not double the foreign kind folder.
 */
public final class VanillaFacingPaths {
    ...
    /**
     * Strips a leading {@code kindFolder/} if present so OI {@code recipeName} stays
     * {@code material/...} under type {@code oi_smelting}.
     */
    ...
    /** OI recipe name under the vanilla-facing type (no kind folder). */
    public static String oiRecipeName(String productPath, String kindFolder) {
        String stripped = stripKindFolder(productPath, kindFolder);
        if (stripped.isBlank()) {
            throw new IllegalArgumentException("empty OI recipe name for kind " + kindFolder + ": " + productPath);
        }
        return stripped;
    }
    ...
    public static String craftingForeignPath(String productPath) {
        ...
        // Already namespaced under shape/shapeless/manual — keep as foreign path and OI name.
        return productPath;
    }
