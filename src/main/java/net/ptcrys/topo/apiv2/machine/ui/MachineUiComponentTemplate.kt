...
object MachineUiComponentTemplate {
     ...
     * ("1mB"/"22B")。供调用方自带角落紧凑数量覆盖层([net.ptcrys.topo.helper.OiCompactNumber] 格式)的
    ...
    fun createTextField(getter: () -> String, setter: (String) -> Unit, placeholder: Component? = null, normalizeForDisplay: (String) -> String = { it }): TextField {
        val field = OiTextField().apply {
            setId("createTextField")
            setAnyString()
            // Match search-pool / amount-editor fields: OI cell chrome, not LDLib RECT_RD_SOLID.
            textFieldStyle {
                it.textColor(MachineUiComponentStyle.textNormal)
                it.cursorColor(MachineUiComponentStyle.textSelected)
                it.textShadow(false)
     ...
     * 由 [net.ptcrys.topo.apiv2.machine.resource.ResourcePort] 在 LEFT 侧栏卡片里挂载。
     */
    fun createSideIoGrid(port: net.ptcrys.topo.apiv2.machine.resource.ResourcePort<*, *>): SideIoConfigGrid = SideIoConfigGrid.create(port)

    /** side-IO 卡的图标标题栏:左 IO 方向箭头 + 右资源图形(悬浮报端口名/trait id)。 */
    fun createSideIoCardTitle(port: net.ptcrys.topo.apiv2.machine.resource.ResourcePort<*, *>): UIElement = SideIoConfigGrid.createTitleBar(port)
}
