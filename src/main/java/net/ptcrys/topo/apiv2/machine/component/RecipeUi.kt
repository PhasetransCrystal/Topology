...
class RecipeUi private constructor(context: ComponentContext<RecipeUi>, private val recipeLogicKey: ComponentKey<RecipeLogic>, private val gate: RecipePageGate) : MachineComponent(context) {
    ...
    override fun collectMachineUi(contribution: MachineUiContribution) {
        contribution.mainPage(
            pageKey,
            BuiltinOIMachineUiLang.UI_RECIPE_PAGE.getComponent(),
            createRecipePage(),
        )
        ...
        contribution.rightPanel(
            statusPanelKey,
            BuiltinOIMachineUiLang.UI_RECIPE_STATUS.getComponent(),
        ) {
            lcdPanel(minValueWidth = LCD_VALUE_MIN_WIDTH) {
                addBoundEntry(
                    BuiltinOIMachineUiLang.UI_RECIPE_STATE.getComponent(),
                    ::stateText,
                    stateLedSupplier(),
                )
                ...
                addPinnedBoundEntry(
                    BuiltinOIMachineUiLang.UI_RECIPE_PROGRESS.getComponent(),
                    ::progressText,
                    LCD_VALUE_MIN_WIDTH,
                    LcdData.LED_RUNNING,
                )
                addBoundEntry(
                    BuiltinOIMachineUiLang.UI_RECIPE_DURATION.getComponent(),
                    ::durationText,
                    LcdData.LED_TEXT,
                )
            }
        ...
        if (modifiers.isNotEmpty()) {
            ...
            contribution.rightPanel(
                modifiersPanelKey,
                BuiltinOIMachineUiLang.UI_RECIPE_MODIFIERS.getComponent(),
    ...
    private fun createModifierTitle(title: Component): UIElement = Label().apply {
        setId("oi_recipe_modifier_title")
        ...
        bar.addEventListener(UIEvents.HOVER_TOOLTIPS) { event ->
            if (XeiRecipeLookup.isAvailable()) {
                event.hoverTooltips = HoverTooltips.create(
                    BuiltinOIMachineUiLang.UI_RECIPE_SHOW_RECIPES.getComponent(),
                )
            }
        }
    }
    ...
    private fun stateText(): Component = if (logic.workMode() == MachineWorkControl.WorkMode.HALTED) {
        BuiltinOIMachineUiLang.UI_RECIPE_STATE_HALTED.getComponent()
    } else {
        ...
        init {
            ...
            addChildren(
                BindableValue(false).apply {
                    setId("oi_recipe_page_gate_open")
                    setDisplay(false)
    ...
    companion object {
        ...
        private fun createGateNotice(noticeText: Component): UIElement {
            ...
            notice.setId("oi_recipe_page_notice")
