...
class SideIoConfigGrid private constructor(defaultPacked: Int, private val serverPacked: () -> Int, private val onCycle: (Direction) -> Unit, private val onReset: () -> Unit, private val onDisable: () -> Unit) : UIElement() {
    ...
    init {
        ...
        addChild(
            row(
                ...
                cell,
                gap,
                controlButton("oi_side_io_reset", BuiltinOIMachineUiLang.UI_SIDE_IO_RESET, MachineUiIcons.reset()) {
                    sendAction(RESET_PAYLOAD)
                },
                faceCell(Direction.UP, "up"),
                controlButton("oi_side_io_disable", BuiltinOIMachineUiLang.UI_SIDE_IO_DISABLE, MachineUiIcons.forbidden()) {
    ...
    private class FaceCell(localSide: Direction, faceKey: String, onClick: () -> Unit) : Button() {
        private val faceName = BuiltinOIMachineUiLang.sideIoFace(faceKey).getComponent()
        private val outerRing = ring("oi_side_io_ring_outer", MachineUiComponentStyle.sideIoRingSize)
        private val innerRing = ring("oi_side_io_ring_inner", MachineUiComponentStyle.sideIoInnerRingSize)

        init {
            setId("oi_side_io_face_${localSide.serializedName}")
            noText()
            val cell = MachineUiComponentStyle.sideIoCellSize
        ...
        fun applyMode(mode: AutomationIo) {
            ...
            style {
                it.tooltips(
                    BuiltinOIMachineUiLang.UI_SIDE_IO_TOOLTIP.getComponent(
                        faceName,
                        BuiltinOIMachineUiLang.sideIoMode(mode).getComponent(),
                    ),
                )
            }
        }
    ...
    companion object {
        ...
        fun createTitleBar(port: ResourcePort<*, *>): UIElement {
            ...
            val portLabel = portDisplayName(
                ...
                BuiltinOIMachineUiLang.sideIoTitle(port.recipeIo()).getComponent(),
            )
            val cell = MachineUiComponentStyle.sideIoCellSize
            val gap = MachineUiComponentStyle.sideIoCellGap
            val gridW = cell * 3f + gap * 2f
            return UIElement().apply {
                setId("oi_side_io_card_title")
                layout {
                    it.flexDirection(FlexDirection.ROW)
                ...
                addChild(titleIcon("oi_side_io_title_io", ioIcon))
                addChild(
                    titleIcon(
                        "oi_side_io_title_resource",
