...
public final class ShellOverlayMachineRenderType extends MachineBlockRenderType<ShellOverlayMachineRenderType.Data> {

    public ShellOverlayMachineRenderType(Identifier id) {
        super(id);
    }

    public MachineBlockRenderUse<Data> overlay(
                                               MachineShellMaterial shell,
                                               Identifier idleFront,
                                               Identifier activeFront) {
        return use(new Data(shell, idleFront, activeFront));
    }
    ...
    public static final class Strategy implements MachineBlockRenderStrategy<Data> {
        ...
        public void validate(Data data) {
            Objects.requireNonNull(data, "shell overlay machine render data");
    ...
    public static final class Data {
        ...
        private final Identifier idleFront;
        ...
        private Data(
            ...
            this.idleFront = Objects.requireNonNull(idleFront, "idle front");
    ...
    private static void generateBlockstate(Block block, RegistryLibBlockModelGenerator prov, Data data) {
        TextureMapping idleMap = textureMapping(data, data.idleFront());
        TextureMapping activeMap = textureMapping(data, data.activeFront());

        Identifier idleModelId = ModelLocationUtils.getModelLocation(block);
        Identifier activeModelId = ModelLocationUtils.getModelLocation(block, "_active");
        prov.withBuilder(machineModelTemplate(), idleMap).build(idleModelId);
        prov.withBuilder(machineModelTemplate(), activeMap).build(activeModelId);

        MultiVariant idleVariant = BlockModelGenerators.plainVariant(idleModelId);
        MultiVariant activeVariant = BlockModelGenerators.plainVariant(activeModelId);

        prov.blockStateOutput.accept(
                MultiVariantGenerator.dispatch(block)
                        .with(PropertyDispatch.initial(OrientedActiveMachineBlock.ACTIVE)
                                .select(false, idleVariant)
                                .select(true, activeVariant))
