...
object OreVeinPanels {
    ...
    fun build(vein: OreVein): UIElement {
        ...
        return MachineUiLayout.box(needPadding = true, needGap = false) {
            root.setId("oi_ore_vein_preview")
            root.layout {
    ...
    private fun buildHeader(vein: OreVein, entries: List<EntryView>): UIElement {
        ...
        return MachineUiLayout.row(
            ...
            id = "oi_ore_vein_header",
        ) {
            if (!icon.isEmpty) {
                add(
                    UIElement().apply {
                        setId("oi_ore_vein_specimen")
                        layout {
    ...
    private fun buildPreviewTile(vein: OreVein, entries: List<EntryView>, total: Int, placement: OrePlacement): UIElement = MachineUiContainerTemplate.createBox(needPadding = true, needGap = false).apply {
        setId("oi_ore_vein_preview_tile")
        layout {
    ...
    private fun buildInfoCard(vein: OreVein, placement: OrePlacement): UIElement = MachineUiLayout.column(gap = 0f, id = "oi_ore_vein_info") {
        ...
        add(infoRow(BuiltinOIOreLang.LABEL_DIM.getComponent(), dimensions(vein), LcdData.LED_TEXT))
        add(
            infoRow(
                BuiltinOIOreLang.LABEL_HEIGHT.getComponent(),
                Component.literal("${vein.minY()}..${vein.maxY()}"),
                LcdData.LED_RUNNING,
            ),
        )
        add(infoRow(BuiltinOIOreLang.LABEL_HOST.getComponent(), hostRocks(vein), LcdData.LED_TEXT))
        add(infoRow(BuiltinOIOreLang.LABEL_SIZE.getComponent(), sizeLabel(placement), LcdData.LED_TEXT))
        add(infoRow(BuiltinOIOreLang.LABEL_SPACING.getComponent(), spacingLabel(placement), LcdData.LED_RUNNING))
        add(
            infoRow(
                BuiltinOIOreLang.LABEL_EXPOSED.getComponent(),
    ...
    private fun buildComposition(entries: List<EntryView>): UIElement {
        ...
        scroller.setId("oi_ore_vein_composition")
        scroller.layout {
    ...
    private fun buildEntryRow(entry: EntryView): UIElement = MachineUiLayout.row(
        ...
        widthPercent = 100f,
        alignItems = AlignItems.CENTER,
        id = "oi_ore_vein_entry_row",
    ) {
        add(buildRowSlot(entry))
        add(
            Label().apply {
                setId("oi_ore_vein_entry_name")
                setText(entry.name)
                textStyle {
        ...
        add(
            Label().apply {
                setId("oi_ore_vein_entry_percent")
    ...
    private fun buildRowSlot(entry: EntryView): ItemSlot {
        ...
        slot.setId("oi_ore_vein_entry_slot")
        slot.layout {
    ...
    private fun materialColor(material: Material): Int = material.strategy().data(BuiltinOIMaterialDataTypes.PRIMARY_COLOR).map { it.rgb() }.orElse(0xFFFFFF)
    ...
    private fun sizeLabel(placement: OrePlacement): Component = when (placement) {
        is OrePlacement.Grid -> BuiltinOIOreLang.SIZE_RADIUS.getComponent(placement.radiusBlocks())
        is OrePlacement.Feature -> BuiltinOIOreLang.SIZE_CLUSTER.getComponent(placement.size())
    }

    private fun spacingLabel(placement: OrePlacement): Component = when (placement) {
        is OrePlacement.Grid -> BuiltinOIOreLang.SPACING_BLOCKS.getComponent(placement.gridSizeChunks() * 16)
        is OrePlacement.Feature -> BuiltinOIOreLang.SPACING_ATTEMPTS.getComponent(placement.attemptsPerChunk())
    }
    ...
    private class CompositionBar(private val color: Int, private val ratio: Double) : UIElement() {
        ...
        init {
            setId("oi_ore_vein_composition_bar")
            layout {
                it.flexGrow(1f)
    ...
    private class ShapePreviewElement(private val entries: List<EntryView>, private val total: Int, private val shape: OreVeinShape, pixelSize: Int) : UIElement() {
        ...
        init {
            setId("oi_ore_vein_shape_preview")
            layout {
    ...
    private class FeaturePreviewElement(private val entries: List<EntryView>, private val total: Int, private val size: Int, private val pixelSize: Int) : UIElement() {
        ...
        init {
            setId("oi_ore_vein_feature_preview")
            layout {
                it.width(pixelSize.toFloat())
                it.height(pixelSize.toFloat())
            }
