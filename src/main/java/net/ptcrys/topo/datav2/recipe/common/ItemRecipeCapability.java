...
import net.ptcrys.topo.apiv2.recipe.search.RecipeSearchKeyRegistry;
...
public final class ItemRecipeCapability extends SlottedRecipeCapability<OIItemInput, OIItemOutput, ItemResource> {

    private static final String RECIPE_ITEM_SLOT_ID = "oi_recipe_item_slot";
    ...
    public ItemRecipeCapability(MachineResourceType<ItemResource> resourceType) {
        super(
                ...
                OIItemInput.CODEC,
                OIItemOutput.CODEC,
                OIItemInput.STREAM_CODEC,
                OIItemOutput.STREAM_CODEC);
    ...
    /** 铸造：精确物品输入。 */
    public RecipeInputUse<OIItemInput> in(ItemLike item, long count) {
        return inputUse(OIItemInput.of(item, count));
    }

    /** 铸造：材料 Form 物品输入（供材料后处理器产配方）。 */
    public RecipeInputUse<OIItemInput> in(Material material, MaterialForm form, long count) {
        return inputUse(OIItemInput.of(MaterialHelper.materialItemSupplier(material, form), count));
    }

    /** 铸造：延迟物品输入（供注册期尚未绑定的物品句柄，如组件 ItemEntry::get）。 */
    public RecipeInputUse<OIItemInput> in(Supplier<? extends ItemLike> item, long count) {
        return inputUse(OIItemInput.of(item, count));
    }

    /** 铸造：物品 tag 输入。 */
    public RecipeInputUse<OIItemInput> inTag(TagKey<Item> tag, long count) {
        return inputUse(OIItemInput.tag(tag, count));
    }

    /** 铸造：催化剂输入（在场不消耗，如工艺模具）。 */
    public RecipeInputUse<OIItemInput> catalyst(ItemLike item) {
        return unconsumedInput(item);
    }

    /** 铸造：催化剂输入的延迟形态，供注册期尚未绑定的物品句柄。 */
    public RecipeInputUse<OIItemInput> catalyst(Supplier<? extends ItemLike> item) {
        return unconsumedInput(item);
    }
     ...
     */
    public RecipeInputUse<OIItemInput> unconsumedInput(ItemLike item) {
        return inputUse(OIItemInput.unconsumed(item));
    }

    /** Deferred unconsumed input for registration-time item suppliers / registry entries. */
    public RecipeInputUse<OIItemInput> unconsumedInput(Supplier<? extends ItemLike> item) {
        return inputUse(OIItemInput.unconsumed(item));
    }

    /** 铸造：精确物品输出。 */
    public RecipeOutputUse<OIItemOutput> out(ItemLike item, long count) {
        return outputUse(OIItemOutput.of(item, count));
    }

    /** 铸造：材料 Form 物品输出。 */
    public RecipeOutputUse<OIItemOutput> out(Material material, MaterialForm form, long count) {
        return outputUse(OIItemOutput.of(MaterialHelper.materialItemSupplier(material, form), count));
    }

    /** 铸造：延迟物品输出（供注册期尚未绑定的物品句柄，如组件 ItemEntry::get）。 */
    public RecipeOutputUse<OIItemOutput> out(Supplier<? extends ItemLike> item, long count) {
        return outputUse(OIItemOutput.of(item, count));
    }

    @Override
    public boolean matchInput(@Nullable MachineBlockEntity machine, List<OIItemInput> contents) {
    ...
    @Override
    public boolean matchOutput(@Nullable MachineBlockEntity machine, List<OIItemOutput> contents) {
    ...
    @Override
    public OIItemInput scaleInput(OIItemInput content, double factor) {
        Objects.requireNonNull(content, "item input");
        if (factor == 1.0d) {
            return content;
        }
        requirePositiveScaleFactor(factor);
        // Sealed exhaustiveness: new OIItemInput subtype must be handled here or fail compile.
        return switch (content) {
            case OIItemInput.Resource resource -> resource.withScaledCount(factor);
            case OIItemInput.Tag tag -> tag.withScaledCount(factor);
            case OIItemInput.AnyOf anyOf -> anyOf.withScaledCount(factor);
            case OIItemInput.Unconsumed unconsumed -> unconsumed; // presence-only; never parallel-multiply
        };
    }

    @Override
    public OIItemInput scaleInputForParallel(OIItemInput content, long factor) {
        return Objects.requireNonNull(content, "item input").withParallelCount(factor);
    }

    @Override
    public long maxParallelScaleInput(OIItemInput content) {
        Objects.requireNonNull(content, "item input");
        return content.consumesOnMatch() ? Long.MAX_VALUE / content.count() : Long.MAX_VALUE;
    }

    @Override
    public OIItemOutput scaleOutput(OIItemOutput content, double factor) {
        Objects.requireNonNull(content, "item output");
        return content.withScaledCount(factor);
    }

    @Override
    public OIItemOutput scaleOutputForParallel(OIItemOutput content, long factor) {
        return Objects.requireNonNull(content, "item output").withParallelCount(factor);
    }

    @Override
    public long maxParallelScaleOutput(OIItemOutput content) {
    ...
    @Override
    public @NonNull UIElement createPreviewInputSlotWidget(@Nullable OIItemInput input) {
    ...
    @Override
    public @NonNull UIElement createPreviewOutputSlotWidget(@Nullable OIItemOutput output) {
        ItemSlot slot = previewSlot();
        if (output == null) {
            return slot;
        }
    ...
    public @NonNull UIElement createLiveSlotWidget(ResourcePort<?, ItemResource> storage, int slot) {
        ...
        itemSlot.setId("oi_live_item_slot_" + slot);
    ...
    public int[] previewInputSlotAssignment(
                                            int slotCount,
                                            List<OIItemInput> startContents,
                                            List<OIItemInput> tickContents) {
        int total = startContents.size() + tickContents.size();
        boolean[] unconsumed = new boolean[total];
        int unconsumedCount = 0;
        for (int i = 0; i < total; i++) {
            OIItemInput content = i < startContents.size() ? startContents.get(i) : tickContents.get(i - startContents.size());
    ...
    private static final class ItemInputPlannerScratch {
        ...
        private boolean reserveUnconsumed(List<OIItemInput> contents, int contentCount) {
            for (int index = 0; index < contentCount; index++) {
                OIItemInput input = contents.get(index);
        ...
        private int buildMatches(List<OIItemInput> contents) {
            ...
            int matchCount = 0;
            for (int input = 0; input < inputCount; input++) {
                OIItemInput ingredient = contents.get(inputContentIndexes[input]);
    ...
    @Override
    public Set<?> indexKeys(OIItemInput content) {
        ...
        return switch (content) {
            case OIItemInput.Resource resource -> Set.of(resource.resource());
            case OIItemInput.Tag tag -> Set.of(tag.tag());
            case OIItemInput.AnyOf ignored -> Set.of();
            // 催化剂在场是必要起始条件,可安全充当搜索剪枝键:没插模具的机器在索引层即被剪掉。
            case OIItemInput.Unconsumed unconsumed -> Set.of(unconsumed.resource());
        };
    }

    @Override
    public long inputAmount(OIItemInput content) {
        return content.count();
    }

    @Override
    public InputIndexAmountMode inputIndexAmountMode(OIItemInput content) {
    ...
    private static boolean insertIntoHandler(
                                             ...
                                             List<OIItemOutput> contents,
                                             Transaction transaction) {
        for (OIItemOutput output : contents) {
