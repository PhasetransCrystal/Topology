...
public final class FluidRecipeCapability
                                         extends SlottedRecipeCapability<OIFluidIngredient, OIFluidIngredient, FluidResource> {

    private static final String RECIPE_FLUID_SLOT_ID = "oi_recipe_fluid_slot";
    ...
    public FluidRecipeCapability(MachineResourceType<FluidResource> resourceType) {
        super(
                ...
                OIFluidIngredient.class,
                OIFluidIngredient.class,
                OIFluidIngredient.CODEC,
                OIFluidIngredient.CODEC,
                OIFluidIngredient.STREAM_CODEC,
                OIFluidIngredient.STREAM_CODEC);
     ...
     */
    public RecipeInputUse<OIFluidIngredient> in(Fluid fluid, long amount) {
        validateAmount(amount);
        return inputUse(new OIFluidIngredient(new FluidStackTemplate(fluid, 1), amount));
    }

    /** Delayed fluid input, for material fluids that are registered by RegistryLib later in bootstrap. */
    public RecipeInputUse<OIFluidIngredient> in(Supplier<? extends Fluid> fluid, long amount) {
        Objects.requireNonNull(fluid, "fluid");
        validateAmount(amount);
        return inputUse(OIFluidIngredient.lazy(
    ...
    public RecipeInputUse<OIFluidIngredient> in(FluidStack stack) {
        return inputUse(new OIFluidIngredient(stack));
    }

    /** 铸造：注册期安全的流体输出；见 {@link #in(Fluid, int)}。 */
    public RecipeOutputUse<OIFluidIngredient> out(Fluid fluid, long amount) {
        validateAmount(amount);
        return outputUse(new OIFluidIngredient(new FluidStackTemplate(fluid, 1), amount));
    }

    /** Delayed fluid output, for material fluids that are registered by RegistryLib later in bootstrap. */
    public RecipeOutputUse<OIFluidIngredient> out(Supplier<? extends Fluid> fluid, long amount) {
        Objects.requireNonNull(fluid, "fluid");
        validateAmount(amount);
        return outputUse(OIFluidIngredient.lazy(
                () -> new FluidStackTemplate(fluid.get(), 1),
                amount,
                new LazyFluidKey(fluid, amount)));
    }
    ...
    public RecipeOutputUse<OIFluidIngredient> out(FluidStack stack) {
        return outputUse(new OIFluidIngredient(stack));
    }
    ...
    @Override
    public boolean matchInput(@Nullable MachineBlockEntity machine, List<OIFluidIngredient> contents) {
    ...
    public long maxParallelByInputs(
                                    ...
                                    List<OIFluidIngredient> contents,
                                    long maxParallel) {
        return maxParallelByInputs(fluidHandler(machine, RecipeRole.INPUT), contents, maxParallel);
    }

    @Override
    public boolean matchOutput(@Nullable MachineBlockEntity machine, List<OIFluidIngredient> contents) {
    ...
    public boolean matchOutputAfterInputs(
                                          @Nullable MachineBlockEntity machine,
                                          List<OIFluidIngredient> inputs,
                                          List<OIFluidIngredient> outputs) {
        if (outputs.isEmpty()) {
            return true;
        }
    ...
    public static boolean matchOutputAfterInputs(
        ...
        for (OIFluidIngredient input : inputs) {
    ...
    @Override
    public OIFluidIngredient scaleInput(OIFluidIngredient content, double factor) {
        return scaleFluid(content, factor);
    }

    @Override
    public OIFluidIngredient scaleOutput(OIFluidIngredient content, double factor) {
        return scaleFluid(content, factor);
    }

    private static OIFluidIngredient scaleFluid(OIFluidIngredient content, double factor) {
        Objects.requireNonNull(content, "fluid content");
        return content.withScaledAmount(factor);
    }

    @Override
    public OIFluidIngredient scaleInputForParallel(OIFluidIngredient content, long factor) {
        return Objects.requireNonNull(content, "fluid input").withParallelAmount(factor);
    }

    @Override
    public OIFluidIngredient scaleOutputForParallel(OIFluidIngredient content, long factor) {
        return Objects.requireNonNull(content, "fluid output").withParallelAmount(factor);
    }

    @Override
    public long maxParallelScaleInput(OIFluidIngredient content) {
        return Long.MAX_VALUE / Objects.requireNonNull(content, "fluid input").amount();
    }

    @Override
    public long maxParallelScaleOutput(OIFluidIngredient content) {
    ...
    @Override
    public @NonNull UIElement createPreviewInputSlotWidget(@Nullable OIFluidIngredient input) {
        return previewFluid(IngredientIO.INPUT, input);
    }

    @Override
    public @NonNull UIElement createPreviewOutputSlotWidget(@Nullable OIFluidIngredient output) {
        return previewFluid(IngredientIO.OUTPUT, output);
    }

    private static UIElement previewFluid(IngredientIO io, @Nullable OIFluidIngredient ingredient) {
    ...
    public @NonNull UIElement createLiveSlotWidget(ResourcePort<?, FluidResource> storage, int slot) {
        ...
        fluidSlot.setId("oi_live_fluid_slot_" + slot);
    ...
    private static final class FluidInputPlannerScratch {
        ...
        private boolean collectDemands(List<OIFluidIngredient> contents, int contentCount) {
            for (int content = 0; content < contentCount; content++) {
                OIFluidIngredient ingredient = contents.get(content);
                if (ingredient == null) {
                    return false;
                }
    ...
    private static boolean consumeFromHandler(
                                              ...
                                              List<OIFluidIngredient> contents,
                                              Transaction transaction) {
        for (OIFluidIngredient ingredient : contents) {
    ...
    private static boolean canInsertOutputs(
                                            ...
                                            long[] slotAmounts) {
        int slotCount = slotResources.length;
        for (OIFluidIngredient output : contents) {
