...
import net.ptcrys.topo.apiv2.machine.ui.tooltip.OiTooltipUiProvider
import net.ptcrys.topo.datav2.equipment.common.StandardEquipmentBase
 ...
 * 物品的化学面板([net.ptcrys.topo.datav2.material.MaterialFormTooltips])。
 *
...
object EquipmentTooltips {
    ...
    fun createPanel(stack: ItemStack, strategy: StandardEquipmentBase): UIElement {
        ...
        lcd.setId("oi_equipment_tooltip")
    ...
    fun panelRows(damageable: Boolean, remainingDurability: Int, maxDurability: Int, tooltipLines: List<EquipmentTooltipLine>, surveyRange: Int? = null): List<Row> {
        ...
        if (damageable) {
            rows += Row(
                BuiltinOIEquipmentLang.TOOLTIP_EQUIPMENT_DURABILITY.getComponent(),
        ...
        if (surveyRange != null) {
            rows += Row(
                BuiltinOIEquipmentLang.TOOLTIP_EQUIPMENT_RANGE.getComponent(),
                BuiltinOIEquipmentLang.TOOLTIP_EQUIPMENT_RANGE_BLOCKS.getComponent(surveyRange),
                false,
            )
        }
        if (tooltipLines.isNotEmpty()) {
            rows += Row(
                BuiltinOIEquipmentLang.TOOLTIP_EQUIPMENT_FUNCTION.getComponent(),
