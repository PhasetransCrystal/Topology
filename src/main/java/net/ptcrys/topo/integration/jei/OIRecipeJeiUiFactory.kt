...
object OIRecipeJeiUiFactory {
    ...
    @JvmStatic
    fun buildPreview(recipeType: OIRecipeType<*>, recipe: OIRecipe): ModularUI {
    ...
    private fun buildLcd(recipe: OIRecipe, scalarRows: List<ScalarContentRow>): LcdData {
        val lcd = MachineUiContainerTemplate.createLcdData(LcdData.Orientation.HORIZONTAL)
            .addStaticIconEntry(
                ...
                LcdData.LED_RUNNING,
                BuiltinOIMachineUiLang.UI_RECIPE_TIME.getComponent(),
            )
    ...
    @JvmStatic
    fun jeiPanelWidth(recipeType: OIRecipeType<*>): Int {
    ...
    private fun collectScalarContents(recipe: OIRecipe): List<ScalarContentRow> {
    ...
    private fun collectScalarInputs(rows: MutableList<ScalarContentRow>, entries: Array<out OIRecipe.InputEntry<*>>, perTick: Boolean) {
    ...
    private fun collectScalarOutputs(rows: MutableList<ScalarContentRow>, entries: Array<out OIRecipe.OutputEntry<*>>, perTick: Boolean) {
        for (entry in entries) {
            val scalar = entry.capability() as? ScalarRecipeCapability ?: continue
            for (content in entry.contents()) {
    ...
    private fun attachScalarBars(rows: List<ScalarContentRow>, durationTicks: Int, ioRow: UIElement): UIElement {
        ...
        val barsRow = UIElement().apply {
            ...
            if (consumed.isNotEmpty() && produced.isNotEmpty()) {
                addChild(
                    UIElement().apply {
                        setId("oi_recipe_scalar_bar_divider")
                        layout {
                            it.width(1f)
        ...
        return UIElement().apply {
            setId("oi_recipe_body_with_scalars")
            layout {
                it.flexDirection(FlexDirection.ROW)
                it.alignItems(AlignItems.CENTER)
                it.justifyContent(AlignContent.CENTER)
    ...
    private fun scalarBar(capability: ScalarRecipeCapability, amount: Long, perTick: Boolean, flow: ResourceBar.Flow, durationTicks: Int): UIElement {
        ...
        bar.setId("oi_recipe_scalar_bar")
        bar.setStaticContent(amount, perTick, flow, durationTicks)
        return bar
    }

    @JvmStatic
    fun jeiPanelHeight(recipeType: OIRecipeType<*>): Int = maxOf(RecipeUiLayout.IO_ROW_HEIGHT + 34, MIN_PANEL_HEIGHT)

    private fun buildJeiSlot(slotCounts: Map<SlottedRecipeCapability<*, *, *>, SlotPlan>, recipe: OIRecipe, io: RecipeRole, globalIndex: Int): UIElement {
    ...
    private fun <I, O> buildTyped(capability: SlottedRecipeCapability<I, O, *>, recipe: OIRecipe, io: RecipeRole, localIndex: Int, slotCount: Int): UIElement {
    ...
    @Suppress("UNCHECKED_CAST")
    private fun <I> inputContents(entries: Array<out OIRecipe.InputEntry<*>>, capability: RecipeCapability<I, *>): List<I> {
        for (entry in entries) {
    ...
    @Suppress("UNCHECKED_CAST")
    private fun <O> outputContents(entries: Array<out OIRecipe.OutputEntry<*>>, capability: RecipeCapability<*, O>): List<O> {
        for (entry in entries) {
            if (entry.capability() === capability) {
