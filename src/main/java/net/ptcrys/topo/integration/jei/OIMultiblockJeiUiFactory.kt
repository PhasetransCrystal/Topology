...
import net.ptcrys.topo.apiv2.machine.multiblock.ui.MultiblockPreviewBlockList
import net.ptcrys.topo.apiv2.machine.multiblock.ui.MultiblockScenePanel
import net.ptcrys.topo.apiv2.machine.multiblock.ui.MultiblockScenePreview
 ...
 */
object OIMultiblockJeiUiFactory {
    ...
    private fun buildPanel(definition: MachineDefinition, expanded: Boolean, width: Int, height: Int): UIElement {
        ...
        return MachineUiLayout.box(needPadding = true, needGap = true) {
            root.setId("oi_multiblock_jei_preview")
            root.layout {
    ...
    private fun infoBar(preview: BlueprintPreview): UIElement = MachineUiComponentTemplate.createStaticText(
        Component.literal(dimensions(preview.blocks().keys) + "  ")
            .append(net.ptcrys.topo.datav2.machine.BuiltinOIMachineUiLang.UI_MULTIBLOCK_PREVIEW_ROTATABLE.getComponent()),
    ).apply {
        setId("oi_multiblock_preview_info")
        layout { it.height(9f) }
    }
